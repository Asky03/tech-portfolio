...
export default function ProjectCard({ project, onOpen, index = 0 }: Props) {
  return (
    <motion.article
      ...
      whileHover={{ y: -6 }}
      className="glow-border group relative flex flex-col rounded-2xl glass cursor-pointer overflow-hidden"
      onClick={onOpen}
    ...
    >
      {project.cover ? (
        ...
        <div className="relative aspect-[16/10] overflow-hidden border-b border-white/[0.06] bg-ink-800/40 flex items-center justify-center">
          <div className="text-white/15 font-mono text-xs tracking-[0.3em] uppercase">
            {project.category}
          </div>
        </div>
      )}
